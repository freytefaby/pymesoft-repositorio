...
<div class="main-content">
						...
						<div class="page-header">
							<h1>
								...
								<small>
									...
									Panel principal notas a credito
								</small>
							</h1>
						</div><!-- /.page-header -->
        ...
        @endif
													{!! Form::open(array('url'=>'peticion/notascredito/create','method'=>'GET', 'autocomplete'=>'off', 'class'=>'form-inline'))!!}
													<div class="col-md-3">
													...
													{!!Form::open(array('url'=>'peticion/notascredito/detalle_nota_credito','method'=>'POST','autocomplete'=>'off','class'=>'form-horizontal','role'=>'form'))!!}
													{{Form::token()}}
														<table class="table">
														<tr>
														<th>Codigo</th>
			                                       ...
			                                       @if($art->idtipoproducto==1)
													  ...
													  @endif
														...
														<input type="hidden" name="idnota" value="{{$ultimanota->idnotacredito}}">
														...
														<input type="hidden" name="stock" value="{{$art->stock}}">
														@endforeach
														</table>
											...
											<div class="widget-box">
												...
												<div class="widget-body">
													<div class="widget-main">
														...
														<th><i class="fa fa-trash" aria-hidden="true"></i> Quitar</th>
														</tr>
														
														@foreach($detallenota as $pro )
														...
														<td><a href="" data-target="#modal-delete-{{$pro->iddetallenotacredito}}" data-toggle="modal" ><button class="btn btn-xs btn-danger" title="Eliminar" >
																<i class="fa fa-trash" aria-hidden="true"></i> Quitar
															</button></a></td>
														</tr>
														 @include('peticion.notascredito.modal')
														@endforeach
														<tfoot>
									...
									{!!Form::model($ultimanota,['method'=>'PATCH','route'=>['peticion.notascredito.update',$ultimanota->idnotacredito],'class'=>'form-horizontal'])!!}
					               {{Form::token()}}
										...
										<div class="col-xs-12 col-sm-4">
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Detalle de nota credito</h4>
                                     ...
                                     @endif
								               ...
								               @endif
														<div>
															...
															<br />
															<select class="chosen-select form-control" name="cliente" id="form-field-select-3" data-placeholder="Seleccione Cliente">
																<option value="1">  </option>
																@foreach($clientes as $client)
																@if(Session::get('cliente')==$client->idcliente)
																	<option value="{{$client->idcliente}}" selected>{{$client->nombrecliente}} {{$client->cedulacliente}}</option>
																@else
																	
																@if(old('cliente')==$client->idcliente)
																<option value="{{$client->idcliente}}" selected>{{$client->nombrecliente}} {{$client->cedulacliente}}</option>
															   @else
																<option value="{{$client->idcliente}}">{{$client->nombrecliente}} {{$client->cedulacliente}}</option>
															   @endif
															   @endif
																@endforeach
															</select>
												...
												<div class="infobox-content">Total a pagar</div>
														...
														<div>
															...
															<div class="input-group">
															   ...
															   <textarea class="form-control" required  name="observacion"  placeholder="Agregar una descripción">{{Session::get('observacion')}}  </textarea>
														   @else
																<textarea  class="form-control"  required name="observacion" placeholder="Agregar una descripción">{{old('observacion')}}   </textarea>
															@endif
                                            ...
                                            <button class="btn btn-xs btn-success" title="Facturar" type="submit">
															...
															<!--IMPUTS-->
															<input type="hidden" name="valorventa" value="{{$sumarray->val}}">
															<input type="hidden" name="subtotal" value="{{$sumarray->sub}}">
															<input type="hidden" name="utilidad" value="{{$sumarray->utilidad}}">
