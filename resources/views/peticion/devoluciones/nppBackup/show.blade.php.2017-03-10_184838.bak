...
<div class="main-content">
						...
						<div class="page-header">
							<h1>
								...
								<small>
									...
									Panel principal y devoluciones a cliente
								</small>
							</h1>
						</div><!-- /.page-header -->
										...
										<div class="col-xs-12 col-sm-8">
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Selección de productos</h4>
        ...
        @endif
													...
													{!! Form::open(array('url'=>'peticion/devoluciones/'.$id,'method'=>'GET', 'autocomplete'=>'off', 'class'=>'form-inline'))!!}
													<div class="col-md-6">
														<select class="chosen-select form-control" name="p" id="form-field-select-3" data-placeholder="Seleccione producto">
																<option value="">  </option>
																
															@foreach($pro as $arti)
															<option value="{{$arti->idproducto}}">{{$arti->nombreproveedor}} {{$arti->descripcionproducto}} {{$arti->codigobarra1}} </option>
															@endforeach
																</select>
													...
													{!!Form::open(array('url'=>'peticion/devoluciones/detalle_devolucion','method'=>'POST','autocomplete'=>'off','class'=>'form-horizontal','role'=>'form'))!!}
													{{Form::token()}}
														<table class="table">
														<tr>
														<th>Codigo</th>
			                                       ...
			                                       @if($art->idtipoproducto==1)
													  ...
													  @endif
														...
														<input type="hidden" name="cantidad_venta" value="{{$art->cantidad}}">
														...
														<input type="hidden" name="comision" value="{{$art->comision}}">
											...
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Detalle de productos agregados</h4>
												...
												<div class="widget-body">
													<div class="widget-main">
														...
														<th><i class="fa fa-barcode" aria-hidden="true"></i> Codigo</th>
														<th><i class="fa fa-product-hunt" aria-hidden="true"></i> Producto</th>
														<th><i class="fa fa-heartbeat" aria-hidden="true"></i> Laboratorio</th>
														<th><i class="fa fa-cart-plus" aria-hidden="true"></i> Cantidad</th>
														<th><i class="fa fa-money" aria-hidden="true"></i> Precio</th>
														...
														<td>{{$pro->cantidad}}</td>
														<td>{{number_format($pro->valor)}}</td>
														<td>{{number_format($pro->subtotal)}}</td>
														...
														<td></td>
														<td></td>
														<td>T: {{$sumarray->cant}}</td>
														<td>T: {{number_format($sumarray->val)}}</td>
														<td>T: {{number_format($sumarray->sub)}}</td>
											...
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Productos en devolución</h4>
												...
												<div class="widget-body">
													<div class="widget-main">
														...
														<th><i class="fa fa-money" aria-hidden="true"></i> Precio</th>
														<th><i class="fa fa-money" aria-hidden="true"></i> Subtotal</th>
														<th><i class="fa fa-money" aria-hidden="true"></i> Eliminar</th>
				
														</tr>
														
														@foreach($detalledevolucion as $pro )
														...
														<td>{{number_format($pro->valor)}}</td>
														<td>{{number_format($pro->subtotal)}}</td>
														<td><a href="" data-target="#modal-delete-{{$pro->iddetalledevolucioncliente}}" data-toggle="modal" ><button class="btn btn-xs btn-danger" title="Eliminar" >
																<i class="fa fa-trash" aria-hidden="true"></i> Quitar
															</button></a></td>
														 @include('peticion.devoluciones.modal')
														@endforeach
														<tfoot>
												
														
														<tr>
														<td>Item: {{$sumarraydev->detalle}} </td>
														<td></td>
														<td></td>
														<td>T: {{$sumarraydev->cant}}</td>
														<td>T: {{number_format($sumarraydev->val)}}</td>
														<td>T: {{number_format($sumarraydev->sub)}}</td>
									...
									{!!Form::open(array('url'=>'peticion/devoluciones','method'=>'POST','autocomplete'=>'off','class'=>'form-horizontal','role'=>'form'))!!}
													{{Form::token()}}
									            <!--VISUALIZACION DE PRODUCTOS -->
										<div class="col-xs-12 col-sm-4">
											<div class="widget-box">
												<div class="widget-header">
													<h4 class="widget-title">Detalle de devolución</h4>
								               ...
								               @endif
														...
														<div>
															...
															<br />
															<h3><p class="text-muted">{{$ventas->nombrecliente}}</p></h3>
															<input type="hidden" name="cliente" value="{{$ventas->idcliente}}">
															<input type="hidden" name="usuario" value="{{$ventas->idusuario}}">
														</div>
														...
														<div>
															...
															<br />
															<h3><p class="text-muted">{{$ventas->desctipoventa}}</p></h3>
														</div>
														<hr/>
														<div>
											...
											<div class="infobox-data">
												<span class="infobox-data-number">{{number_format($sumarraydev->val)}}</span>
												<div class="infobox-content">Devolución</div>
											</div>
                                          ...
                                          <div>
															<label for="form-field-select-3">Observaciones</label>

															<br />
															<textarea name="observacion" placeholder="describe motivo por le devolución" class="form-control"></textarea>
														</div>
														
														<hr/>
														<button class="btn btn-xs btn-success" title="Facturar" type="submit">
																<i class="ace-icon fa fa-check bigger-110"></i> Generar devolución
															</button>
															<!--IMPUTS-->
															<input type="hidden" name="valorventa" value="{{$sumarraydev->val}}">
															<input type="hidden" name="subtotal" value="{{$sumarraydev->sub}}">
															<input type="hidden" name="utilidad" value="{{$sumarraydev->utilidad}}">
															<input type="hidden" name="comision" value="{{$sumarraydev->comision}}">
